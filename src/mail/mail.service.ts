...
export class MailService {
  ...
  public async sendEmail(
    ...
    const url = `${baseUrl}/${lang}/auth/verify-email?email=${encodeURIComponent(email)}&id=${encodeURIComponent(id)}${redirect ? `&redirect=${encodeURIComponent(redirect)}` : ''}&token=${encodeURIComponent(token)}`;
